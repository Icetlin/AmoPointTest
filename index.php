<?php
// Проверяем, был ли отправлен POST-запрос (если форма была отправлена)
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    $uploadDir = __DIR__ . '/files/'; // Путь к папке files
    $uploadFile = $uploadDir . basename($file['name']); // Полный путь к файлу

    // Перемещаем загруженный файл из временной директории в нашу папку "files"
    if (move_uploaded_file($file['tmp_name'], $uploadFile)) {
        // Если файл успешно загружен, выводим зеленый круг
        ...
        foreach ($lines as $line) {
            ...
            // Используем регулярное выражение для подсчета цифр в строке
            preg_match_all('/\d/', $line, $matches); // Находим все цифры в строке
